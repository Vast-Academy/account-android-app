...
const AccountDetailScreen = ({route, navigation}) => {
  ...
  const handleWithdraw = async () => {
    const parsedAmount = parseAmountValue(withdrawAmount);
    ...
    setLoading(true);
    try {
      const amountValue = Math.abs(parsedAmount);
    ...
    return (
      <View style={styles.chatList}>
        {transactions.map(txn => {
          ...
          const txnAmount = parseAmountValue(txn.amount) || 0;
          ...
          return (
            <View key={txn.id}>
              ...
              <View
                style={[
                  styles.chatRow,
                  txnAmount < 0 && styles.chatRowDebit,
                ]}>
                <View
                  style={[
                    styles.chatBubble,
                    txnAmount > 0 && styles.chatBubbleCredit,
                    txnAmount < 0 && styles.chatBubbleDebit,
                  ]}>
                  <View style={styles.chatHeader}>
                    <View
                      style={[
                        styles.chatIcon,
                        txnAmount < 0 && styles.chatIconDebit,
                      ]}>
                      <Icon
                        name={txnAmount < 0 ? 'arrow-down' : 'arrow-up'}
                        size={16}
                        color={txnAmount < 0 ? '#EF4444' : '#10B981'}
                      />
                    </View>
                    <Text
                      style={[
                        styles.chatAmount,
                        txnAmount < 0 && styles.chatAmountDebit,
                      ]}>
                      {(txnAmount < 0 ? '-' : '+') +
                ...
                <View
                  style={[
                    styles.chatMeta,
                    txnAmount < 0 && styles.chatMetaDebit,
                  ]}>
